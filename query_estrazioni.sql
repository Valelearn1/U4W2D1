...
-- 3. Estrarre il numero delle fatture con iva al 20% (quante sono?)
...
-- 7. Considerando soltanto le fatture con iva al 20 per cento, estrarre il numero di fatture (conteggio fatture) per ogni anno
...
SELECT EXTRACT(YEAR FROM data_fattura) AS anno, COUNT(*) AS numero_fatture, SUM(importo) AS totale_importo
FROM fatture
GROUP BY EXTRACT(YEAR FROM data_fattura)
ORDER BY anno;